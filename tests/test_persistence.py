...
class TestPersistGame:
    def test_returns_false_when_no_key(self):
        with patch("darwin.persistence._SUPABASE_KEY", ""):
            result = persist_game("test_id", MagicMock(), MagicMock())
            assert result is False

    def test_returns_false_on_exception(self):
        with patch("darwin.persistence._get_client") as mock_client:
    ...
    def test_graceful_on_none_client(self):
        with patch("darwin.persistence._get_client", return_value=None):
            result = persist_game("test_id", MagicMock(), MagicMock())
            assert result is False
