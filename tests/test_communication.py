...
class TestParseCommunications:
    def test_valid_json(self):
        ...
        msgs = parse_communications(raw, "atlas", "Atlas", "House Clair", 1, VALID_NAMES)
    ...
    def test_empty_response_is_silence(self):
        msgs = parse_communications("", "atlas", "Atlas", "House Clair", 1, VALID_NAMES)
        assert len(msgs) == 0
    ...
    def test_message_fields_populated(self):
        ...
        msgs = parse_communications(raw, "atlas", "Atlas", "House Clair", 3, VALID_NAMES)
        assert len(msgs) == 1
        m = msgs[0]
        assert m.round == 3
...
class TestCommunicationManager:
    ...
    def test_last_round_dm_routing(self):
        ...
        # Atlas should see the DM to them, not the one to Raze
...
class TestParseAction:
    def test_valid_stay_json(self):
        raw = '{"action": "stay"}'
        action = parse_action(raw, "atlas", VALID_NAMES)
